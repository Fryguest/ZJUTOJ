...
if (isset($_GET['cid'])){
	...
	$sql_lock="SELECT `start_time`,`title`,`end_time` FROM `contest` WHERE `contest_id`='$cid'";
        ...
        $time_sql="";
        if(time()>$lock_time&&time()<$end_time) $lock=true;
        else $lock=false;
	...
	if(isset($_SESSION['administrator'])||isset($_SESSION['source_browser'])||(isset($_SESSION['user_id'])&&(isset($_GET['user_id'])&&$_GET['user_id']==$_SESSION['user_id']))){
		if ($_SESSION['user_id']!="guest") $sql="SELECT * FROM `solution` WHERE contest_id is null ";
	}else $sql="SELECT * FROM `solution` WHERE problem_id>0 and contest_id is null ";
}
$start_first=true;
